    ...
    return new Promise ((resolve,reject) => {
      try{
        ...
        if (exists) {
          ...
          if (!obj[member.guild.id]) resolve(name);
...
client.on('guildCreate', guild => {
  ...
  .addField('**Restart Bot sunucunuza eklendi!**', `${owner}`)
  .addField('**Restart Bot** sunucunuzdaki insanlara kolaylıklar sağlar.', `**${prefix}bilgi** yazmanız yeterlidir!`)
...
client.on('guildCreate', guild => {
  ...
  .setFooter('Restart Bot', client.user.avatarURL)
  .setTimestamp()
  client.channels.get('507280036278173706').send(embed);
});
...
client.on('message', async msg => {
	...
	if (msg.author.bot) return name;
	if (!msg.content.startsWith(prefix)) return name;
