...
const emoji7 = message.client.emojis.get('📞');
      let isEnabled;
      message.reply("Canlı Destek Komutunu Kullandığınız İçin Teşekkürler. Birazdan Yetkili Bir Kişi Bağlanıcak");
      let mesaj = args.slice(0).join(' ');
      let chan = message.channel;
      let destekKanal = "505402681754517505";
    ...
    collector.on('end', (collected, reason) => {
      ...
      if (reason === 'success') {
        ...
        client.channels.get(destekKanal).send('📞 Destek Çağrısını Kapatmak İçin `kapat` Yazınız.')
